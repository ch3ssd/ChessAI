...
class ChessPieceModel:
    def __init__(self, drive_url: str = None, config_path: str = "config.properties", device: torch.device = None):
        # Load config properties from the config file
        self.config = self.load_config(config_path)

        # Set up the device and other parameters
        self.device = device or torch.device("cuda" if torch.cuda.is_available() else "cpu")
        self.data_dir = Path(self.config["DATA"]["DataDirectory"]).resolve()
        self.batch_size = int(self.config["MODEL"]["BatchSize"])
